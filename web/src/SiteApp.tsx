  ...
  return <>
    ...
    <main>
      ...
      <section className="section shell" id="team" data-source="site_assets">
        <div className="section-heading"><span className="eyebrow">01 / The team</span><h2>The people behind the pipeline.</h2><p>Member names and links are loaded from <code>site_assets/team.json</code>, sourced from the project README.</p></div>
        {assets ? <>
      ...
      <section className="section shell" id="report" data-source="site_assets"><div className="section-heading"><span className="eyebrow">06 / Technical report</span><h2>Findings that changed the system.</h2><p>The report cards below are rendered from <code>site_assets/eda/format_findings.json</code>, with limitations from <code>site_assets/failures/failures.json</code>.</p></div>{!assets ? <EvidenceLoading error={assetError} /> : <><div className="decision-grid">{assets.format.decisions.map((item, index) => <article key={item.finding}><span>{String(index + 1).padStart(2, "0")}</span><h3>{item.finding}</h3><p>{item.decision}</p></article>)}</div><div className="resource-row" id="links"><a href={SOURCE_URL} target="_blank" rel="noreferrer">Source repository ↗</a><a href={LIVE_DEMO_URL} target="_blank" rel="noreferrer">Live demo ↗</a><span>{assets.format.measured_costs_x_realtime.budget}</span></div></>}</section>
    </main>
